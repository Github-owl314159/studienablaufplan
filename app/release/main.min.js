    ...
    */
		...
		const app = () => {
			...
			function addModule() {
				...
				$(".grid-container").append(`<a href='#' class='modules' id='module-${i}'><p>Wahlpflichtfach</p></a>`);
				$(`#module-${i}`).on("click", () => {
					...
					if ($(currentModule).text() === "Wahlpflichtfach") {
						...
						$("#sM-name").append(`<p>${$(currentModule).attr("data-moduleName")}</p>`);
			...
			function confirm(evt) {
				...
				if (modulName && ects && modulName.search(",") === -1 && !duplicateExists(modulName)) {
					// AJAX call
					$.ajax({
						url: "/modules",
						type: "POST",
						...
						success: function () {
							let currentModule = $(".confirmButton").parents("#addModule").attr("data-module");
					...
					if (!$(".reqWarning").length) {
						$("#aM-content").append("<p class='reqWarning' style='margin-top: 20%;'>Please fill in all fields and no special characters are allowed.</p>");
					}
				}
			}
			...
			function changeText() {
				...
				$("#schedule-name").replaceWith(`<input id='schedule-name-input' type='text' value='${content}'>`);
				$("#schedule-name-input").focus();
				$("#schedule-name-input").css("font-size", "145%").css("margin", "3% 0 3% 0").css("font-weight", "bold").on("focusout keypress", evt => {
					if (evt.type === "focusout" || evt.keyCode === 13) {
						let content = $("#schedule-name-input").val();
						$("#schedule-name-input").replaceWith(`<p id='schedule-name'>${content}</p>`);
					}
				}).on("change", () => {
					console.log("change");
				});
			...
			function changeTextSM() {
				...
				$("#sM-name p").replaceWith(`<input id='sM-input' type='text' value='${content}'>`);
				$("#sM-input").focus();
				$("#sM-input").css("font-weight", "bold").on("focusout keypress", evt => {
					if (evt.type === "focusout" || evt.keyCode === 13) {
          ...
          <div class="scheduleModal-entry" id="schedule-${i}">
			...
			function cancelScheduleModal() {
				...
				$("#scheduleModal-list").children().remove();
